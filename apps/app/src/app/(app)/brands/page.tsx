...
export default async function BrandsPage() {
  ...
  return (
    <>
      ...
      <PageHeader
        title="Brand kits"
        description="One source of truth for your logos, colors, typography, and creative voice."
        actions={
          <Link href="/brands/new" className="btn btn-orange">
      ...
      <div className="collection-toolbar">
        <span className="collection-count">
          {ctx.brands.length} brand{ctx.brands.length === 1 ? "" : "s"} in{" "}
          {ctx.org.name}
        </span>
        <span className="collection-count">
          Select a kit to manage brand guidelines
        </span>
      </div>
      ...
      {ctx.brands.length ? (
            ...
            return (
              <div key={b.id} className="tile flex flex-col">
                <Link
                  ...
                  style={{
                    background: c?.background ?? "#f8f7f3",
                    color: c?.text ?? "#242521",
                  }}
                >
                  ...
                  <div className="flex flex-col gap-2">
                    <span
                      className="text-[22px] leading-[1.05] tracking-[-0.6px]"
                      style={{
                        fontFamily: fontStack(kit?.fonts.heading ?? "DM Sans"),
                        color: c?.primary,
                      }}
                    >
                      {kit?.tagline || "Your brand starts here."}
                    </span>
                    <span
                      className="inline-flex h-7 w-fit items-center px-3 text-[11px] font-semibold"
                      style={{
                        background:
                          kit?.ctaStyle === "outline"
                            ? "transparent"
                            : (c?.accent ?? "#e65c32"),
                        color:
                          kit?.ctaStyle === "outline"
                ...
                <div className="flex items-center justify-between gap-3 px-[13px] pb-[13px] pt-3">
                  <div className="flex min-w-0 flex-col gap-1">
                    ...
                    <span className="truncate text-[11px] text-muted">
                      {[b.industry, b.website?.replace(/^https?:\/\//, "")]
                        .filter(Boolean)
                        .join(" · ") || "No details yet"}
                    </span>
                  </div>
        ...
        <div className="panel flex flex-col items-start gap-3 border-dashed p-6">
          ...
          <p className="m-0 max-w-[52ch] text-[13px] text-muted">
            A brand holds its products, colours, fonts and tone. Every brief and
            creative belongs to one.
          </p>
