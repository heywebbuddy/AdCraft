  ...
  return <form action={createCreativeFromConcept} className={styles.studio}>
    ...
    <div className={styles.main}>
      <section className={styles.section}>
        ...
        <div className={styles.modes}>
          {([
            ["ai", "spark", "AI-designed ad", "Let AI compose the whole picture.", "Photography, typography and layout, designed together."],
            ...
            <span className={styles.modeIcon} aria-hidden="true">{icon === "spark" ? <Spark size={22} /> : icon}</span><span className={styles.modeTitle}>{title}{id === "ai" && <small>RECOMMENDED</small>}</span>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.templates}>
          {directions.map((d) => <label key={d.id} className={`${styles.template} ${template === d.id ? styles.chosen : ""}`}>
            ...
            <span className={styles.templateTitle}>{d.title}<OutArrowIcon /></span><span className={styles.templateNote}>{d.note}</span>
          </label>)}
        </div>
    ...
    <aside className={styles.aside}>
      <section className={styles.brief}><div className={styles.briefHeading}><span className="eyebrow">YOUR CREATIVE BRIEF</span><span className={styles.liveDot}>Ready</span></div>
        ...
        <div className={styles.briefCopy}><span className={styles.label}>THE MESSAGE</span><h3>{concept.headline}</h3><p>{concept.hook}</p><span className={styles.cta}>{concept.cta}<OutArrowIcon /></span></div>
      ...
      <div className={styles.asideNote}><Spark size={18} aria-hidden="true" /><div><strong>A starting point. Never a limit.</strong><p>{mode === "ai" ? "Fine-tune the result with image edits, then share it for review before launch." : "Every text and brand layer stays editable. Make it yours in the editor."}</p></div></div>
