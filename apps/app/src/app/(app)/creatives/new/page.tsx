...
export default async function NewCreativePage({ searchParams }: { searchParams: Promise<{ conceptId?: string; error?: string; detail?: string }> }) {
  ...
  if (!concept) return <>
    ...
    <section className="panel flex flex-col items-start gap-4 p-7"><p className="m-0 max-w-[55ch] text-[13px] text-muted">Choose a concept in a creative brief. Its message, product and brand kit come with you into the studio.</p><Link className="btn btn-dark h-11" href="/briefs">Choose a creative brief ↗</Link></section>
  </>;
