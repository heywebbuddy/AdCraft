...
import {
  getConceptForCreative,
  imageModelChoices,
  STATIC_PLACEMENT_IDS,
...
const ERRORS: Record<string, string> = {
  ...
  concept:
    "That concept could not be turned into a creative. Try again from the brief.",
};
...
export default async function NewCreativePage({
  ...
  searchParams: Promise<{ conceptId?: string; error?: string }>;
}) {
  const ctx = await requireOrg();
  const { conceptId, error } = await searchParams;
  const concept = conceptId
    ? await getConceptForCreative(ctx.org.id, conceptId)
    : null;
  const models = imageModelChoices();
  const kit = concept?.brand.kit;
  const saved = await listTemplatesFor(
    ctx.org.id,
    ctx.brand?.id ?? null,
    "static",
  );
  const colors = {
    primary: kit?.colors.primary ?? "#242521",
      ...
      <>
        ...
        <div className="panel flex flex-col items-start gap-3 border-dashed p-6">
          <p className="m-0 max-w-[52ch] text-[13px] text-muted">
            A creative is a concept made real: its headline, CTA and visual
            direction, painted by an image model and laid out by a template in
            every size. Open a brief and choose the concept you want to make.
          </p>
      ...
      >
        ...
        <div className="panel flex flex-col gap-7 p-6">
          <div className="flex flex-col gap-3">
            <div className="flex items-baseline justify-between gap-3">
              ...
              <span className="text-[11px] text-muted">
                Text, logo and product are exact in every size. You can switch
                later.
              </span>
            </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {models.map((m) => (
                <label
                  key={m.id}
                  className={`${optionClass} min-h-[84px] justify-between`}
                >
                  <input
                    type="radio"
                    name="model"
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-line pt-5">
            ...
            <span className="text-[12px] text-muted">
              About twenty seconds. Sizes render one after another.
            </span>
          </div>
        </div>
        ...
        <aside className="flex flex-col gap-4">
          <section className="panel flex flex-col gap-2.5 px-4 py-3.5">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[15px] font-semibold leading-snug tracking-[-0.3px]">
            ...
            <span
              className="inline-flex w-fit rounded-full px-3 py-1 text-[12px] font-semibold text-white"
            ...
            <ul className="m-0 flex list-none flex-col gap-1.5 p-0 text-[13px]">
              ...
              <li className="flex justify-between gap-2">
                ...
                <span className="text-muted">
                  {kit?.logoUrl ? "From kit" : "Brand name"}
                </span>
              </li>
              <li className="flex justify-between gap-2">
                <span>Product</span>
                <span className="text-muted">
                  {concept.product
                    ? concept.product.cutoutKey
                      ...
                      : concept.product.imageKey
                        ? `${concept.product.name} · no cutout yet`
                        : "No photo"
                    : "None"}
                </span>
              </li>
          ...
          {concept.existingCreatives > 0 ? (
            <p className="m-0 px-1 text-[12px] text-muted">
              This concept already has {concept.existingCreatives} creative
              {concept.existingCreatives === 1 ? "" : "s"}.{" "}
