    ...
    <div className={`lp-root ${size === "page" ? "lp-page" : ""}`}>
      ...
      <div className="lp-grid" role="listbox" aria-label="Look packs">
        ...
        {onBrowse && <div role="option" tabIndex={0} aria-selected={state.packId === "remix"} className="lp-card lp-remix" onClick={() => { onChange({ packId: "remix" }); if (!state.remix) onBrowse(); }} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onChange({ packId: "remix" }); if (!state.remix) onBrowse(); } }}>
          <span className="lp-swatch lp-preview lp-swatch-remix" aria-hidden="true">{state.remix?.previewUrl ? <img src={state.remix.previewUrl} alt="" /> : <span>↗</span>}<span className="lp-looks">2 looks</span></span>
          <span className="lp-card-text" title="Pick any of the 25,000 presenter looks; HeyGen restyles your character into it."><strong>{state.remix ? state.remix.name : "Remix a library look"}</strong>{state.remix ? <button type="button" className="cs-text-button" onClick={e => { e.stopPropagation(); onBrowse(); }}>Change ↗</button> : <small>From the presenter library</small>}</span>
        </div>}
