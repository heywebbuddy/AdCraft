    ...
    <div className={`lp-root ${size === "page" ? "lp-page" : ""}`}>
      ...
      <div className="lp-grid" role="listbox" aria-label="Look packs">
        {packs.map(pk => <div key={pk.id} role="option" tabIndex={0} aria-selected={state.packId === pk.id} className="lp-card" onClick={() => pick(pk.id)} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); pick(pk.id); } }}>
          <span className="lp-swatch lp-preview" aria-hidden="true" style={{ background: `linear-gradient(90deg, ${pk.palette.join(",")})` }}><img src={pk.previews[state.gender]} alt="" loading="lazy" /><span className="lp-looks">{pk.looks} {pk.looks === 1 ? "look" : "looks"}</span></span>
          <span className="lp-card-text" title={pk.description}><strong>{pk.name}</strong><small>{pk.kind === "style" ? "Style" : "Role"}</small></span>
        </div>)}
        {onBrowse && <div role="option" tabIndex={0} aria-selected={state.packId === "remix"} className="lp-card lp-remix" onClick={() => { onChange({ packId: "remix" }); if (!state.remix) onBrowse(); }} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onChange({ packId: "remix" }); if (!state.remix) onBrowse(); } }}>
          <span className="lp-swatch lp-preview lp-swatch-remix" aria-hidden="true">{state.remix?.previewUrl ? <img src={state.remix.previewUrl} alt="" /> : <span>↗︎</span>}<span className="lp-looks">2 looks</span></span>
          <span className="lp-card-text" title="Pick any of the 25,000 presenter looks; HeyGen restyles your character into it."><strong>{state.remix ? state.remix.name : "Remix a library look"}</strong>{state.remix ? <button type="button" className="cs-text-button" onClick={e => { e.stopPropagation(); onBrowse(); }}>Change ↗︎</button> : <small>From the presenter library</small>}</span>
        </div>}
        <div role="option" tabIndex={0} aria-selected={state.packId === "prompt"} className="lp-card lp-prompt" onClick={() => pick("prompt")} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); pick("prompt"); } }}>
          <span className="lp-swatch lp-preview lp-swatch-prompt" aria-hidden="true">Aa<span className="lp-looks">1 look</span></span>
          <span className="lp-card-text" title="Describe an outfit and setting; HeyGen keeps the face."><strong>Your own words</strong><small>Prompt</small></span>
        </div>
      </div>
