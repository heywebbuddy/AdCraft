...
import { isElevenLabsConfigured, isFalConfigured, isHeyGenConfigured, listVoices, videoModels } from "@adcraft/ai";
import { requireOrg } from "@/server/org";
import { studioModels } from "@/server/character-studio";
import { currentSubscription, CREDIT_COSTS } from "@/server/billing";
import { getModelOverrides, planAllows } from "@/server/platform-settings";
...
export async function createCharacterAdAction(form: FormData): Promise<{ creativeId?: string; error?: string }> {
  try {
    ...
    if (!videoModels.some(m => m.id === model) || overrides[model]?.enabled === false) throw new Error("Choose an available video model.");
