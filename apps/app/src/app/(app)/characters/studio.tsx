...
import { MakerLogo, inferMaker, providerLabel } from "@/components/maker-logo";
...
export function CharacterStudio({ data, brandName, balance, videoCredits, canEdit, planEnabled }: Props) {
  ...
  return <div className="character-studio">
    ...
    {view === "studio" && <div className="cs-workspace"><div className="cs-controls">
      <section className="cs-section"><div className="cs-section-heading"><span>01</span><h2>Your character</h2><button type="button" className="cs-text-button" onClick={() => setView("characters")}>Manage cast ↗︎</button></div>
      ...
      <div className="cs-character-grid">{data.characters.map(c => <article className={`cs-character-card ${["queued", "generating"].includes(c.status) ? "busy" : ""}`} key={c.id}>
        ...
        <div className="cs-cast-info">
          ...
          <div className="cs-cast-actions">
            <button type="button" className="cs-secondary" disabled={!c.portraitUrl} onClick={() => { setCharacterId(c.id); setLookId(""); setStockAvatar(null); setView("studio"); }}>Use in an ad ↗︎</button>
