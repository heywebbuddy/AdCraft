    ...
    {view === "studio" && <div className="cs-workspace"><div className="cs-controls">
      <section className="cs-section"><div className="cs-section-heading"><span>01</span><h2>Your character</h2><button type="button" className="cs-text-button" onClick={() => setView("characters")}>Manage cast ↗</button></div>
        ...
        {stockAvatar ? <>
          ...
          <div className="cs-voice-row"><span>♫ {voice?.label ?? "Saved voice"} · {({ low: "calm", medium: "natural", high: "energetic" } as const)[character.motion?.expressiveness ?? "high"]} motion</span><button type="button" className="cs-text-button" disabled={!canEdit} onClick={() => openCharacter("profile")}>Edit profile</button></div>{voice?.previewUrl && <AudioPreview src={voice.previewUrl} label={`Preview ${voice.label}`} compact />}
