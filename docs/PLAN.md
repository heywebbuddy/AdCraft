...
## 17. Voice tools and HeyGen look packs

- **Voice library → Clone your voice**: one recording (≤10 MB) → `POST /v3/voices/clone` (instant clone, Starfish engine). The result is a *private* HeyGen voice; `brand_voices` scopes it to the brand that made it. Trains in ~10–60 s; the UI polls `GET /v3/voices/{id}` and copies the preview into storage (`org/<org>/voices/<id>.mp3`) because HeyGen previews are signed URLs.
- **Voice library → Design a voice**: description → `POST /v3/voices` returns up to three voices that HeyGen has *already created* as private voices. The user keeps one (`brand_voices`, kind `designed`); the rest are deleted so they don't eat the account's clone slots (10 by default). Closing the dialog without keeping deletes the batch. At the time of writing HeyGen returns nothing for many female prompts — the UI says so and suggests rephrasing.
- The shared catalogue (`listHeyGenVoices`) excludes every private voice on the account; `searchVoicesAction` prepends the workspace's own voices and `resolveVoice(orgId, id)` checks them before the catalogue.
- **New look → HeyGen look packs**: the character's portrait is registered once as a HeyGen photo avatar (`POST /v3/avatars type photo`, stored in `characters.heygen`), then `POST /v3/avatars/looks` applies a Look Pack (5 looks) or template (2), or `POST /v3/avatars type prompt` makes one from words. Job `character.looks` polls the looks, copies previews into storage and stores each with its `heygenLookId`; ads using such a look render via `POST /v3/videos` (`avatar_id`, Avatar V motion) instead of an image upload. Credits: `CREDIT_COSTS.heygenLook` per look, refunded pro rata for failures. Each documented template id resolves as a look via `GET /v3/avatars/looks/{id}` with a `preview_image_url`, so pack cards show HeyGen's own thumbnails (`LOOK_PACKS[].previews`). The two documented single templates are ordinary public-library looks (Margot / Kacper), so **Remix a library look** lets any of the ~25k presenter-library looks be used as a `type: "template"` (2 looks) — reachable from the Design-a-look gallery and from the Presenter library's selection bar ("Remix for my character"). Not yet exercised against HeyGen (credits); if HeyGen rejects a non-template look id it returns 400 and the credits are refunded.

## 18. New character: generated face, HeyGen photo/prompt avatar, or digital twin

Character studio's navigation lives in the sidebar: **Create an ad · My characters · Presenter library · Voice library · Design a look** (`/characters?view=…`; ad templates are a sub-view of Create an ad). The in-page tab bar is gone. Every way to make a character is one **New character** dialog with three sources — *Generate a face* (our image models, as before), *From a photo / Describe them for HeyGen*, *Clone a real person* — so there is no separate "avatar" concept for users. Cast cards show a HeyGen / Digital twin badge and, for twins, the consent state with Copy consent link.

- **Clone a real person** — footage (15 s – 10 min, one person, audible speech; up to 600 MB) is uploaded via `POST /api/uploads` (route handler; server actions cap at 16 MB) into org storage, then the `avatar.create` job pushes it to HeyGen (`/v3/assets` multipart under 32 MB, presigned direct upload above) and calls `POST /v3/avatars type digital_twin`. HeyGen clones the voice from the same footage (`avatar_item.default_voice_id`) — recorded in `brand_voices` as a clone and set as the character's voice once ready. Twins need the subject's consent: the job issues `POST /v3/avatars/{group}/consent` (webcam flow, link valid 24 h, stored in `characters.heygen.consentUrl`); the page offers "Copy consent link" / "Check status" (`twinConsentAction`). Consent upload (level 2) and waiver (level 3) are enterprise-only on HeyGen. 25 credits.
- **Create a virtual character** — from a photo (`type photo`) or a description (`type prompt`), voice chosen in the dialog. 5 credits. No consent step.

Both land as `characters` rows with `heygen.type`, the HeyGen look as portrait + first look (`heygenLookId`), so look packs and ads work on them immediately (ads render through `POST /v3/videos` with Avatar V). Verified end-to-end for the photo path (upload → HeyGen photo avatar → character ready in ~40 s); the twin path is built to the docs but untested (needs real footage + the subject's consent recording).
